...
public class Java7ParallelAggregator implements Aggregator {
    ...
    public List<Pair<String, Long>> getMostFrequentWords(List<String> words, long limit) {
        ...
        frequencyList.sort(new Comparator<Pair<String, Long>>() {
            @Override
        ...
        protected void compute() {
            if ((words.size()) < THRESHOLD) {
                for(String word: words) {
                    ...
                    Java7ParallelAggregator.tempFrequencyMap.put(word,
                            wordOccurrenceCounter == null ? 1 : wordOccurrenceCounter + 1);
                }
            } else {
                FrequencyRecursiveAction t1 = new FrequencyRecursiveAction(words.subList(0, words.size()/2));
                FrequencyRecursiveAction t2 = new FrequencyRecursiveAction(words.subList(words.size()/2, words.size()));
                t1.fork().join();
                t2.fork().join();
            }
        }
        ...
        protected void compute() {
            if ((words.size()) < THRESHOLD) {
                for(String word: words) {
                    ...
                    Java7ParallelAggregator.tempDuplicatesMap.put(word,
                            wordOccurrenceCounter == null ? 1 : wordOccurrenceCounter + 1);
                }
            } else {
                DuplicatesRecursiveAction t1 = new DuplicatesRecursiveAction(words.subList(0, words.size()/2));
                DuplicatesRecursiveAction t2 = new DuplicatesRecursiveAction(words.subList(words.size()/2, words.size()));
                t1.fork().join();
                t2.fork().join();
            }
        }
